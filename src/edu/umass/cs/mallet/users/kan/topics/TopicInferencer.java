...
public class TopicInferencer implements Serializable {
    ...
    public TopicInferencer (TypeTopicCounts typeTopicCounts, int[] tokensPerTopic, Alphabet alphabet,
        ...
        numTypes = typeTopicCounts.getNumTypes();
    ...
    public double[] getSampledDistribution(Instance instance, int numIterations,
        ...
        for (int position = 0; position < docLength; position++) {
            ...
            if (typeTopicCounts.typeExists(type)) {
                ...
                topics[position] = typeTopicCounts.getTypeTopicCount(type, 0);
        ...
        for (int iteration = 1; iteration <= numIterations; iteration++) {
            ...
            for (int position = 0; position < docLength; position++) {
                ...
                // ignore out-of-vocabulary terms
                if (!typeTopicCounts.typeExists(type)) { continue; }
                ...
                topicTermMass = typeTopicCounts.calculateTopicTermScores(type, cachedCoefficients, topicTermScores);
                ...
                if (sample < topicTermMass) {
                    ...
                    newTopic =  typeTopicCounts.getTypeTopicCount(type, i);     // currentTypeTopicCounts[i] & topicMask;
                }
                else {
                    ...
                    if (sample < topicBetaMass) {
                        ...
                        while (sample > 0.0) {
                            ...
                            if (newTopic >= numTopics) {
                                typeTopicCounts.debugPrint(System.out, type, topicTermScores, cachedCoefficients);
                            }
